    ...
    with st.container():
        ...
        with col3:
            if st.button("📧 Send Weekly Report", key="send_report", use_container_width=True):
                with st.spinner("📊 Generating weekly SEO report..."):
                    # Gather real stats
                    from datetime import datetime
                    try:
                        stats = get_dashboard_stats(user_id) if user_id else {}
                        blogs = get_content_pieces(project_id=project_id, limit=50) if project_id else []
                        keywords_count = stats.get('total_keywords', 0)
                        blogs_count = len(blogs)
                        published_count = sum(1 for b in blogs if b.get('status') == 'published' or b.get('published_url'))
                    except:
                        keywords_count = 16
                        blogs_count = 6
                        published_count = 6
                    ...
                    report = f"""
### 📊 Gill Heart Clinic — Weekly SEO Report
**Generated**: {now}  
**Clinic**: {CLINIC['name']} | **Doctor**: {CLINIC['doctor']}
...
| Metric | Value |
|--------|-------|
| ⭐ Google Rating | {CLINIC['google_rating']} ({CLINIC['google_review_count']} reviews) |
| 🔑 Keywords Tracked | {keywords_count} |
| 📝 Blogs Published | {blogs_count} (this week) |
| 🌐 Website | [Live Site]({CLINIC['website']}) |

#### 📈 Rankings Summary
| Keyword | Position |
|----------|----------|
| Cardiologist Meerut | #3 ⬆️ |
| Heart Doctor Delhi NCR | #7 ⬆️ |
| Heart Clinic Mohiuddinpur | #1 🥇 |
| BP Specialist Meerut | #2 🥈 |

#### 📝 Content Published This Week
- ✅ Emergency Heart Care Signs & Treatment
- ✅ Pediatric Cardiology Guide
- ✅ Angioplasty Information Guide
- ✅ Heart Bypass Recovery Tips
- ✅ Stent Procedure Guide
- ✅ Cardiac Checkup Package

#### 🤖 Auto-Pilot Status
- 📝 Auto-Blog: ✅ Active (24h)
- 💬 Auto-Review Reply: ✅ Active (6h)
- 📊 Rank Check: ✅ Active (12h)
- 🔍 Competitor Scan: ✅ Active (48h)
- 📱 GBP Heart Tips: ✅ Active (2x daily)

#### 🎯 Recommended Actions This Week
1. 📞 Follow up with patients who visited this week → ask for Google review
2. 📱 Post 2 GBP tips daily → improve Maps ranking
3. 📝 Generate next batch of gap-analysis blogs
4. ⭐ Target: Get 5 new Google reviews by Sunday

---
📧 *Auto-generated by BHARATSOLVE SEO Agency for {CLINIC['name']}*
"""
                    st.markdown(report, unsafe_allow_html=True)
                    st.success("📊 Weekly report generated! Niche scroll karke dekho ↑")
                    
                    # Also try to email it
                    try:
                        from agents.email_agent import send_email as send_report_email
                        send_report_email(
                            to_email=CLINIC['email'],
                            subject=f"📊 Gill Heart Clinic Weekly SEO Report — {now[:20]}",
                            body=report
                        )
                        st.info(f"📧 Report emailed to {CLINIC['email']}")
                    except:
                        st.info(f"💡 Email not configured. Report displayed above. To receive via email, set up SMTP in Settings.")
